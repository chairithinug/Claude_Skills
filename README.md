...
| [`system-design-advising/`](system-design-advising/) | `/system-design-advising` | Senior software-architect consultation for non-AI system design — greenfield, review, scale-diagnose, decision-support, and quick modes. Forces context elicitation before prescription; names trade-offs, anti-patterns, and an operability bar (deploy / observability / on-call / monthly cost) on every recommendation. Resists canned best-practice dumps and ivory-tower architecture as explicit failure modes. 9 reference files split by concern (layered analysis, decision tree, API matrix, resilience, scaling diagnostics, anti-patterns, operability, cost anchors, worked example) and 2 take-away templates in `assets/` (ADR + design doc). Ships with trigger eval (20 cases) and quality evals (3 mode-stress scenarios). |
...
### `system-design-advising`
- **Strength** — explicit failure-mode prevention (canned best-practice dump, ivory-tower architecture) baked into the workflow rather than implicit; mode routing (quick / greenfield / review / scale-diagnose / decision-support) sized to question type so a 2-paragraph question gets a 2-paragraph answer; constraint elicitation forced before prescription; every recommendation paired with explicit trade-off and operability bar (deploy / observability / on-call / monthly cost). 9 reference files split by concern and 2 take-away templates in `assets/` make this the first repo skill to formalize the references-vs-assets distinction (Claude reads vs. user takes — STANDARDS §7).
- **Weakness** — non-AI focus is a real boundary the user has to recognize (AI / RAG / agent design routes to `/ai-solution-advising`); the line is sometimes blurry for ML/data infra (a feature store may live on either side depending on framing). Body is the second-longest in the repo at ≈315 lines — for a quick clarification a smaller skill would feel less heavy.
- **Opportunity** — could chain with `/ai-solution-advising` (when an AI-native system has a non-AI infrastructure layer that needs designing), with `/deciding` (when context is fully established and a final pick is what's needed), and with `/devil-advocating` for stress-testing a proposed architecture before commitment. The `assets/` template pattern could spread to other skills that ship deliverables (interview-rubric templates, postmortem skeletons, runbook scaffolds).
- **Threat** — like all advisory skills, false precision: naming concrete numbers and specific failure modes can read more authoritative than the framework deserves on novel architectures or fast-moving regimes (cloud pricing, managed-service feature parity). The `references/cost-anchors.md` file is especially time-sensitive — re-verifying when used is part of using it correctly.
